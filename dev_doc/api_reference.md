...
通过 EN 引脚对蜂窝模组断电再上电，并重新执行 `modemInit()`。这是比 `AT+CFUN=1,1` 更彻底的模组重启方式，但不会重启 ESP32-C3 主控。

### `void checkScheduledModemRestart()`

按北京时间检查定时重启配置。支持每天、每周、每月以及 4G 软重启、4G 硬重启、整机重启三种方式。函数在执行前持久化当天运行标记，避免同一分钟重复触发；执行结果通过已启用的通知通道发送。

当前支持三种模式：4G 软重启、4G EN 硬重启、4G 硬重启后再执行 `ESP.restart()` 的整机重启。

### `void sendSystemNotification(NotificationRouteType route, ...)`

根据系统消息分类和推送规则选择邮箱 1～3及推送通道 1～5。正则过滤开启时仅发送匹配规则的消息，多规则命中时合并通道并去重。

### `void sendRoutedIncomingNotification(...)`

用于收到短信和来电提醒。匹配文本由号码与正文组合，之后分别发送到规则选中的邮箱和推送通道。
**行为**:
...
### `void handleRoot()`
以分段方式返回 SPA 主页 HTML，逐个替换模板占位符并流式生成 5 个推送通道表单，避免一次性复制整页造成内存不足。配置区包含系统概览、账号管理、WiFi 设置、邮件通道、推送通道、推送规则和定时任务；管理员与黑名单内容合并到账号管理。工具区保留发送短信、模组诊断、网络测试、模组控制、AT 终端和系统日志。
