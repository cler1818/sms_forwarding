...
网页入口位于“配置 → 定时任务 → 定时模组重启”。软重启和硬重启只控制蜂窝模组；整机重启会同时重启蜂窝模组和 ESP32-C3。

新增第三种 `MODEM_RESTART_WHOLE_DEVICE`：先执行 `resetModule()` 硬重启 4G 模组，再调用 `ESP.restart()` 重启 ESP32-C3。执行前先发送整机重启提示，重新启动联网后由开机通知确认上线。

### 推送规则路由

`NotificationRouteConfig` 为系统消息、短信、电话、其他四类消息分别保存默认邮箱掩码、默认推送通道掩码、正则过滤开关和 5 条匹配规则。规则数据使用 `rule2_0`～`rule2_3` 序列化为单个 NVS 字符串，避免与早期九分类测试数据混用。

系统通知在格式化前调用 `sendSystemNotification(route, subject, body)`；收到短信和来电调用 `sendRoutedIncomingNotification()`。规则匹配基于原始标题、号码和正文，多个匹配规则的目标掩码按位合并，天然去重。

正则过滤和所有规则默认关闭，规则 1 仅预填示例。开启过滤时服务器强制要求至少一条启用规则。内置轻量匹配器支持常用量词、字符组、锚点、转义字符和 `|` 多分支，按 UTF-8 字节匹配中文字面量。

主页通过 DOM 布局迁移将管理员与黑名单卡片合并进账号管理，并把定时短信、状态快照、定时重启集中到新的“定时任务”面板。配置区菜单依次为账号管理、WiFi 设置、邮件通道、推送通道、推送规则、定时任务；规则内嵌页只输出规则表单，不输出完整主页，避免右侧重复出现侧栏。

主页 `%PUSH_CHANNELS%` 不再通过一次性 `String.replace()` 扩展整页，而是通过 `WebServer.sendContent()` 分段发送页面前缀、通道表单和页面后缀，避免连续堆内存不足导致占位符原样显示。占位符值为空时必须跳过 `sendContent()`；分块响应中的空数据块代表响应结束，若提前发送会截断后续 HTML 和 JavaScript。
