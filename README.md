...
- **推送规则**：系统消息、收到的短信、来电和其他兜底消息可分别选择邮箱 1～3、推送通道 1～5，并支持多条正则匹配规则。
...
“定时任务”页面支持配置每天、每周或每月定时重启，并可选择以下方式：

- **软重启**：发送 `AT+CFUN=1,1`，主要重启模组协议栈和固件，耗时较短，对供电冲击较小。
- **硬重启**：通过 ESP32-C3 控制模组 EN 引脚断电再上电，重启更彻底，适合模组无 AT 响应或软重启无效的情况。
- **整机重启**：先通过 EN 引脚硬重启 4G 模组，再执行 `ESP.restart()` 重启 ESP32-C3 主控，是范围最完整的重启方式。

软重启和硬重启只控制蜂窝通信模组；只有“整机重启”会同时重启 4G 模组和 ESP32-C3。任务执行前会先保存当天的运行标记，避免同一分钟内重复触发。定时功能依赖设备已获得有效网络时间，默认关闭。

### 推送规则

左侧“推送规则”将通知合并为四类：系统消息、短信、电话、其他。开机、WiFi 恢复、配置变化、重启、状态快照和短信发送结果均归入系统消息；“其他”作为以后新增消息的防漏兜底。每一类都可独立选择邮箱 1～3和推送通道 1～5。

每类消息提供默认通道和 5 个正则规则槽位。正则过滤默认关闭，所有规则也默认不勾选，只预填示例内容；短信规则 1 预填验证码相关表达式。开启正则过滤时必须至少勾选一条规则，否则拒绝保存。多条规则同时匹配时会合并目标通道并去重。

“管理员 & 黑名单”已合并到“账号管理”的登录凭据下方。左侧新增“定时任务”，集中显示定时短信、状态快照和定时模组/整机重启；原“定时发送”更名为“定时短信”。

左侧“配置”区当前顺序为：使用说明、系统概览、账号管理、WiFi 设置、邮件通道、推送通道、推送规则、定时任务。“定时任务”不再放在“工具”区。推送规则使用独立内容页嵌入右侧，避免重复显示整套左侧菜单。

轻量正则支持 `|`、`.`、`*`、`+`、`?`、`^`、`$`、字符组 `[]`、`\d`、`\w`、`\s`，英文匹配不区分大小写。规则保存在 NVS 中。

管理主页的推送通道表单采用分段流式输出，避免页面较大、堆内存碎片化时 `%PUSH_CHANNELS%` 占位符替换失败。空白配置项会直接跳过输出，不能作为空 HTTP 数据块发送，否则浏览器会把它当成页面传输结束，造成页面脚本缺失和左侧菜单无法点击。
