...
- **WiFi 自动恢复**：配网热点与 STA 同时运行，断线后持续尝试三组 WiFi；恢复并连续稳定 60 秒后发送“WiFi断线后已重新上线”提醒，10 分钟内重复波动只提醒一次。
- **定时模组重启**：可在“模组控制”中配置每天、每周或每月定时重启，并选择 AT 软重启或 EN 引脚硬重启。
...
设备支持三组 WiFi 按主 WiFi、备用 A、备用 B 的顺序连接。运行期间 WiFi 中断时，设备会开启 `192.168.4.1` 配网热点，同时继续在后台自动重连，不会因一次连接失败永久停留在热点模式。

配网页支持实时重新扫描和手动输入 SSID。后台连接使用全信道扫描，并在重新连接 STA 前恢复正常发射功率。

当运行中的设备发生 WiFi 中断并重新上线后，会等待连接连续稳定 60 秒，再通过已启用的推送/邮箱通道发送恢复提醒。提醒包含 SSID、IP、RSSI、BSSID、信道、估算离线时长和断线原因代码。为避免路由器波动造成消息轰炸，恢复提醒设置了 10 分钟冷却时间；冷却期内的重复断线恢复不会再次提醒。设备首次开机联网仍发送原有开机通知，不作为断线恢复提醒。

![](assets/screenshots/wifi_settings.png)

### 定时模组重启

“模组控制”页面支持配置每天、每周或每月定时重启 4G/LTE 模组，并可选择以下方式：

- **软重启**：发送 `AT+CFUN=1,1`，主要重启模组协议栈和固件，耗时较短，对供电冲击较小。
- **硬重启**：通过 ESP32-C3 控制模组 EN 引脚断电再上电，重启更彻底，适合模组无 AT 响应或软重启无效的情况。

定时重启仅重启蜂窝通信模组，不会重启 ESP32-C3 主控。任务执行前会先保存当天的运行标记，避免同一分钟内重复触发；执行完成后会通过已配置的推送/邮箱通道发送结果通知。定时功能依赖设备已获得有效网络时间，默认关闭。
